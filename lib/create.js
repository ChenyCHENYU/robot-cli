...
async function selectBySearch() {
  while (true) {
   ...
   ${chalk.cyan(techInfo)} ${chalk.dim(`• 模板key: ${key}`)}
   ${chalk.dim('─'.repeat(60))}`,
        value: { key, ...template },
        short: template.name,
      };
...
async function selectFromAll() {
  ...
  Object.entries(allTemplates)
    ...
    .forEach(([key, template]) => {
     ...
     ${chalk.dim(template.description)}
     ${chalk.cyan(`技术栈: ${techStack}`)} ${chalk.dim(`• 命令: robot create my-app -t ${key}`)}`,
        value: { key, ...template },
        short: template.name
      });
    });
  ...
  Object.entries(allTemplates)
    ...
    .forEach(([key, template]) => {
      ...
      categorizedChoices.push({
        name: `  ● ${chalk.bold(template.name)} ${versionTag}
     ${chalk.dim(template.description)}
     ${chalk.cyan('跨平台: 小程序/H5/App')} ${chalk.dim(`• 命令: robot create my-app -t ${key}`)}`,
        value: { key, ...template },
        short: template.name
      });
    });
  ...
  Object.entries(allTemplates)
    ...
    .forEach(([key, template]) => {
      ...
      categorizedChoices.push({
        name: `  ● ${chalk.bold(template.name)} ${versionTag}
     ${chalk.dim(template.description)}
     ${chalk.cyan(`框架: ${framework}`)} ${chalk.dim(`• 命令: robot create my-app -t ${key}`)}`,
        value: { key, ...template },
        short: template.name
      });
    });
